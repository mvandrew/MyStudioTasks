...
Функция РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(КодКалендаря, НомерГода, Знач КодБазовогоКалендаря = Неопределено) Экспорт
	...
	ДанныеИзМакета = ДанныеПроизводственныхКалендарейИзМакета(КодыКалендарей, Ложь);
...
Функция РезультатЗаполненияПроизводственныхКалендарейПоУмолчанию(КодыКалендарей) Экспорт
	...
	ДанныеИзМакета = ДанныеПроизводственныхКалендарейИзМакета(КодыДанныхМакета, Ложь);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НомераГодов.Количество() = 0 Тогда
			// Заполняем по умолчанию на текущий год.
			НомераГодов.Добавить(Год(ТекущаяДатаСеанса()));
		КонецЕсли;
...
Функция ДанныеПроизводственныхКалендарейИзМакета(КодыКалендарей = Неопределено, ФормироватьПолныйНабор = Истина) Экспорт
	...
	Возврат ДанныеПроизводственныхКалендарейИзXML(ТекстовыйДокумент.ПолучитьТекст(), КодыКалендарей, ФормироватьПолныйНабор);
...
//	* ДатаПереноса.
//
Функция ДанныеПроизводственныхКалендарейИзXML(Знач XML, КодыКалендарей = Неопределено, ФормироватьПолныйНабор = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			Если ЗначениеЗаполнено(Выборка.КодБазового) Тогда
				// Зависимые календари не создаем автоматически при обновлении.
				Продолжить;
			КонецЕсли;
...
Процедура ОбновитьДанныеЗависимыхПроизводственныхКалендарей(ИзмененияКалендарей) Экспорт
	...
	ДанныеИзМакета = ДанныеПроизводственныхКалендарейИзМакета(КодыЗависимых, Ложь);
...
Функция КодБазовогоКалендаря(КодКалендаря, КлассификаторКалендарей = Неопределено)
	
	Если КлассификаторКалендарей = Неопределено Тогда
		КлассификаторКалендарей = КалендарныеГрафики.ТаблицаПроизводственныхКалендарей();
	КонецЕсли;
...
Функция НоваяСтрокаДанныхКалендаряИзКлассификатора(ДанныеКалендаря, СтрокаКлассификатора, Проверять = Ложь, Замещать = Ложь)
	...
	Если Проверять Тогда
		...
		ЗаполнитьЗначенияСвойств(ОтборСтрок, СтрокаКлассификатора);
