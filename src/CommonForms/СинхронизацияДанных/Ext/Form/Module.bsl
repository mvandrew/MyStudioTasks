...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	ВариантНастройки = ОбменДаннымиСервер.СохраненныйВариантНастройкиУзлаПланаОбмена(НастроеннаяСинхронизация.УзелИнформационнойБазы);
	
	Заголовок = ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(
		НастроеннаяСинхронизация.УзелИнформационнойБазы.Метаданные().Имя,
		"ЗаголовокУзлаПланаОбмена",
		ВариантНастройки);
	
	ЕстьПравоНаПросмотрЖурналаРегистрации = ПравоДоступа("ЖурналРегистрации", Метаданные);
...
Процедура ДобавитьКомандыСозданияНовогоОбмена()
	...
	Для Каждого Элемент Из СписокПлановОбмена Цикл
		...
		ЗначенияНастроекПланаОбмена = ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(ИмяПланаОбмена,
									"ВариантыНастроекОбмена, ВидГруппыДляВариантовНастроек");
		ВозможныеНастройкиОбмена = ЗначенияНастроекПланаОбмена.ВариантыНастроекОбмена;
		ВидГруппыДляВариантовНастроек = ЗначенияНастроекПланаОбмена.ВидГруппыДляВариантовНастроек;
			
		СуммирующаяГруппа = РодительскаяГруппа;

		СуммирующаяГруппаДобавлена = Ложь;
		...
		Для Каждого ВариантНастройки Из ВозможныеНастройкиОбмена Цикл
			...
			Если Не СуммирующаяГруппаДобавлена И ВозможныеНастройкиОбмена.Количество() > 1 Тогда
				ВидГруппыКнопок = ВидГруппыДляВариантовНастроек;
				...
				Если ОбменДаннымиСервер.ЭтоПланОбменаXDTO(ИмяПланаОбмена) Тогда
					СуммирующаяГруппа.Заголовок = НСтр("ru = 'Через универсальный формат обмена'");
				Иначе
					СуммирующаяГруппа.Заголовок = ЗначенияНастроекДляВарианта.НаименованиеКонфигурацииКорреспондента;
				КонецЕсли;
				
				СуммирующаяГруппаДобавлена = Истина;
			КонецЕсли;
			
			Если ВариантНастройки.КорреспондентВЛокальномРежиме Тогда
				НаименованиеКоманды = ИмяПланаОбмена + "ИдентификаторНастройки" + ПредопределеннаяНастройка;
				ЗаголовокКоманды    = ЗначенияНастроекДляВарианта.ЗаголовокКомандыДляСозданияНовогоОбменаДанными;
				СоздатьКомандуИЭлементФормы(НаименованиеКоманды, ЗаголовокКоманды, СуммирующаяГруппа);
			КонецЕсли;
			
			Если ВариантНастройки.КорреспондентВМоделиСервиса Тогда
