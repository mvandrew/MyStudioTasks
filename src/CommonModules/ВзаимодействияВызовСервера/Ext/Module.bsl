...
Функция ОтправитьПолучитьПочтуПользователяВФоне(УникальныйИдентификатор) Экспорт
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"ВзаимодействияВызовСервера.ОтправитьЗагрузитьПочтуПользователя",
			ПараметрыПроцедуры,
			ПараметрыВыполнения);
...
//
Функция ОтправитьЗагрузитьПочтуПользователя(ПараметрыВыгрузки, АдресХранилища) Экспорт
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка, ИмяДокумента) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 50 РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
