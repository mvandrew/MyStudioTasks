...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДанныеКлассификатораДополнительныеТерритории = Обработки.РасширенныйВводКонтактнойИнформации.АдресаДляИнтерактивногоВыбора(Родитель, 90, ПараметрыПоиска);
...
Процедура НайтиАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если НЕ ПустаяСтрока(Текст) Тогда
		...
		РезультатПодбора = ЗаполнитьПорциюВариантовПоПервойБукве(Текст, Родитель);
		ВариантыАдреса.Очистить();
		
		Для каждого Адрес Из РезультатПодбора Цикл
	...
	Иначе
		ПодключитьОбработчикОжидания("ПерейтиВНачалоСписка", 0.1, Истина);
	КонецЕсли;
...
Процедура ВариантыАдресаПриАктивизацииСтроки(Элемент)
	Элементы.ВыбратьУлицу.Доступность = (Элемент.ТекущиеДанные <> Неопределено);
...
Процедура ЗаполнитьЧастьАдресаПоПредставлению(НаименованиеЧастиАдреса, Представление, Идентификатор)
	...
	НаименованиеСокращение= УправлениеКонтактнойИнформациейКлиентСервер.НаименованиеСокращение(Представление);
	...
	Если НаименованиеЧастиАдреса = "ДополнительныйЭлемент" Тогда
		НаселенныйПунктДетально[НаименованиеЧастиАдреса].ПутьXPath = РаботаСАдресамиКлиентСервер.XPathДополнительногоОбъектаАдресации(90, НаименованиеСокращение.Сокращение);
	ИначеЕсли НаименованиеЧастиАдреса = "ПодчиненныйЭлемент" Тогда
		НаселенныйПунктДетально[НаименованиеЧастиАдреса].ПутьXPath = РаботаСАдресамиКлиентСервер.XPathДополнительногоОбъектаАдресации(91);
	Иначе
...
Функция ЗаполнитьПорциюВариантовПоПервойБукве(Текст, Родитель)
	...
	ДанныеКлассификатора = Обработки.РасширенныйВводКонтактнойИнформации.СписокАвтоподбораУлицы(Родитель, Текст, ДополнительныеПараметры);
	Если ДанныеКлассификатора.Отказ Тогда
		Возврат ДанныеВыбора;
	КонецЕсли;
...
Процедура УстановитьДанныеПоПредставлениюУлицы(Представление, ИдентификаторРодителя)
	...
	Иначе
		...
		АнализКлассификатора =Обработки.РасширенныйВводКонтактнойИнформации.УлицыПоПредставлению(ИдентификаторРодителя, Представление);
...
Функция ЕстьПодчиненныеЭлементы(Идентификатор, Уровень = 90)
	...
	ДанныеКлассификатора = Обработки.РасширенныйВводКонтактнойИнформации.АдресаДляИнтерактивногоВыбора(Идентификатор, Уровень, ПараметрыПоиска);
	Если ДанныеКлассификатора.Данные.Количество() > 0 Тогда
		Возврат Истина;
	КонецЕсли;
